...
export class App extends gfx.GfxApp
{
    ...
    createScene(): void 
    {
        ...
        for(let i=0; i < this.starfield.numParticles; i++)
        {
            ...
            console.log(this.starfield.particleSizes[i]);
        }
    ...
    update(deltaTime: number): void 
    {
        ...
        for(let i=0; i < this.starfield.numParticles; i++)
        {
           ...
           // ADD YOUR CODE HERE
           starPosition.x += moveDirection.x * shipSpeed * -10000 * deltaTime * this.starfield.particleSizes[i];
           starPosition.y += moveDirection.y * shipSpeed* -10000 * deltaTime * this.starfield.particleSizes[i];
        ...
        this.mines.children.forEach((mine: gfx.Node2) => {
            ...
            // ADD YOUR CODE HERE
            const shipDirection = new gfx.Vector2(0, shipSpeed * -1);
            shipDirection.rotate(this.ship.rotation)
            mine.position.x +=  shipDirection.x ;
            mine.position.y +=  shipDirection.y ;

            mine.rotation += mineRotation; // rotation
        ...
        this.lasers.children.forEach((laser: gfx.Node2) => {
            ...
            // ADD YOUR CODE HERE
            const laserDirection = new gfx.Vector2(0, laserSpeed);
            laserDirection.rotate(laser.rotation);
            laser.position.x += laserDirection.x;
            laser.position.y += laserDirection.y;
    
            // Check if the laser is outside the scene boundaries
    ...
    onMouseDown(event: MouseEvent): void 
    {
      ...
      // Create a new laser instance using Mesh2 directly, since ShapeInstance is deprecated
      const newLaser = gfx.Geometry2Factory.createBox();

      // Creating laser with the dimensions and color wanted 
      newLaser.material.color.set(.247, .995, .284, 1); // bright green color to be the same as the project
      newLaser.scale.x = 0.01; // scale set to create a long, thin beam
      newLaser.scale.y = 0.05;
  
      // Position the new laser at the ship's current position
      newLaser.position.copy(this.ship.position);
  
      // Rotate the new laser to point towards the mouse cursor
      // This aligns the laser's direction with the ship's direction towards the cursor
      newLaser.lookAt(this.mousePosition);
  
      // Add the new laser to the lasers group
